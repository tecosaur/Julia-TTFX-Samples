...
issue_checkboxes = (;
    taskdir = (done = Ref(false), desc = "Create task directory"),
    taskenv = (done = Ref(false), desc = "Initialise task environment"),
    taskscript = (done = Ref(false), desc = "Create task script"),
    taskrun = (done = Ref(false), desc = "Run task script"),
    taskjulia = (done = Ref(false), desc = "Determine minimum Julia version"))

function issue_comment()
    cbox(item::@NamedTuple{done::Base.RefValue{Bool}, desc::String}) =
...
issue_checkboxes.taskdir.done[] = true
...
issue_checkboxes.taskenv.done[] = true
update_issue_comment()
...
issue_checkboxes.taskscript.done[] = true
update_issue_comment()
...
println(gh_output, "task_time_using=", tasktimes[1])
println(gh_output, "task_time_script=", tasktimes[2])
println(gh_output, "task_time_total=", tasktimes[3])

issue_checkboxes.taskrun.done[] = true
update_issue_comment()
...
issue_checkboxes.taskjulia.done[] = true
update_issue_comment()
@info "Finishing up"
